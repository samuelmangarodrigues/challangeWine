...
import { ButtonPagination, ButtonSearch, ContainerPagination, ContainerSearch, ContainerStore, DivItems, FindProducts, InputSearch, ContainerPaginationAndItems} from "./styled"
...
const Loja = () => {
    ...
    const{getItems,currentItems,pages,setCurrentPage,filteredProds,filterProductsFunc} = useItem()
    ...
    <>
        <ContainerStore>
            <ContainerSearch>
                ...
                <ButtonSearch onClick={()=>{filterProductsFunc(input as string)}}>
                    <IoSearchCircleOutline size={31.66}color="#D14B8F"/>
                </ButtonSearch>
            </ContainerSearch>
            {input !== "" && filteredProds.length !== 62 && filteredProds.length !== 0?<FindProducts><strong>{filteredProds.length}</strong>produtos encontrados</FindProducts>:false}
            <ToastContainer/>
